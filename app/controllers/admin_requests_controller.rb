...
class AdminRequestsController < ApplicationController
  ...
  def new
    ...
    if @has_request
      respond_to do |format|
        format.html do
          redirect_to(admin_requests_path, notice: 'You do not have access to create requests. You can only have one in progress at a time.')
        end
        format.json { head(:no_content) }
      end
  ...
  def create
    ...
    if @has_request
      respond_to do |format|
        format.html do
          redirect_to(admin_requests_path, notice: 'You do not have access to create more requests. You are currently have a request in progress.')
        end
        format.json { head(:no_content) }
      end
  ...
  def has_admin_request
    ...
    @has_request = AdminRequest.all.count do |req|
